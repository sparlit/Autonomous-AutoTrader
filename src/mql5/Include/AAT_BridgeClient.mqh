...
public:
   ...
   void OnTick() {
      uint n=GetTickCount();
      if(!m_s.IsConnected()) { m_s.Connect(m_h, m_p); m_syn=false; if(n-m_l_hb>60000) ActFS(); return; }
      ...
      if((n-m_l_tk > 200) && (m_l_pr==0 || MathAbs(cp-m_l_pr)>=m_p_th || n-m_l_dp>60000)) {
         if(m_s.Send(CAATProtocol::BuildDATA_PUSH(_Symbol, _Period, 30, ++m_seq_tx))) {
            m_l_dp=n; m_l_pr=cp; m_l_tk=n;
         }
      }
