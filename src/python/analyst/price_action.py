...
class SMCAnalyst:
    ...
    def detect_market_structure(self, df: pd.DataFrame, atr: float = 0.0) -> Dict[str, Any]:
        """Magic: 8001"""
        if len(df) < 15: return {"trend": "NEUTRAL", "choch": False, "sweep": False, "swing_h": None, "swing_l": None, "m_id": 8001}
        ...
        return {"trend": trend, "choch": choch, "sweep": sweep, "swing_h": highs[-1] if len(highs) > 0 else None, "swing_l": lows[-1] if len(lows) > 0 else None, "m_id": 8001}
    ...
    def detect_order_blocks(self, df: pd.DataFrame, atr: float = 0.0) -> List[Dict[str, Any]]:
        ...
        obs = []
        for idx in np.where(bullish_ob_mask)[0]: obs.append({"type": "BULLISH", "top": h[idx], "bottom": l[idx], "index": int(idx), "m_id": 8002})
        for idx in np.where(bearish_ob_mask)[0]: obs.append({"type": "BEARISH", "top": h[idx], "bottom": l[idx], "index": int(idx), "m_id": 8002})
        return obs[-5:]
