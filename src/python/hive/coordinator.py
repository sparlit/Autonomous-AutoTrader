    ...
    def _normalize_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if m_type == "HB":
            return {
                ...
                "equity": message.get("e"),
                "drawdown": message.get("d")
            }
        if m_type == "DP":
            raw_h = message.get("h", [])
    ...
    async def handle_message(self, client_id: str, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if msg_type == "DATA_PUSH":
            ...
            if "draw" in decision_maker:
                response["drw"] = decision_maker["draw"]

            if decision_maker.get("action") in ["BUY", "SELL"]:
                validation = self.risk_manager.validate_trade(symbol, decision_maker["action"], equity, atr=atr)
                if validation["safe"]:
                    response["act"] = validation["action"]
                    response["lts"] = validation["lots"]
                else:
