...
class RiskManager:
    ...
    def is_news_safe(self) -> bool:
        """
        11004: Enhanced News safety window check.
        30-minute blackout before and after high-impact events (NFP, FOMC).
        """
        now = datetime.datetime.now(datetime.UTC)
        for event in self.news_events:
            try:
                # Expecting ISO format from Forex Factory scraper or manual json
    ...
    def calculate_trade_params(self, equity: float, atr: float, symbol: str, action: str, tick_val: float = 10.0, tick_size: float = 0.0001) -> Dict[str, Any]:
        ...
        # Novice-safe 1% risk rule
        risk_c = equity * (self.config.risk.risk_per_trade_pct / 100.0)
        sl_dist = atr * 2 # Standard 2x ATR Stop
        ...
        return {
            ...
            "tp_pts": int((sl_dist * 2) / tick_size) # 1:2 RR
        }
    ...
    def validate_trade(self, symbol: str, action: str, current_equity: float, atr: float = 0.0, spread: float = 0.0, tick_val: float = 10.0, tick_size: float = 0.0001, ignore_session: bool = False) -> Dict[str, Any]:
        ...
        # Spread Blowout Protection: No trade if spread > 3x average or > 50% ATR
        if atr > 0 and spread > atr * 0.5: return {"safe": False, "reason": "SPREAD_BLOWOUT"}

        # 11007: Peak Equity & Drawdown Protection
        ...
        # 11008: Correlation & One-Trade Rule per Symbol
        # In this master-pro mode, we enforce one trade per symbol
