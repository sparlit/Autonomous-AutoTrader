...
class BaseBrain(Process, BrainContract):
    ...
    async def _main_loop(self):
        """12007: Async execution loop with throttled polling to prevent Manager saturation."""
        ...
        while self.is_running:
            try:
                ...
                if self.ipc:
                    messages = self.ipc.xread({stream_name: '0'}, count=20)
                    if messages:
                        for stream, msgs in messages:
                            for msg_id, data in msgs:
                                ...
                                try:
                                    ...
                                    logger.error(f"Brain {self.name} TIMEOUT. Dropping result.")
                                self._latency_sum += (time.perf_counter() - start_time)
                                self._processed_count += 1
                        await asyncio.sleep(0.005) # Minor breath between batches
                    else:
                        await asyncio.sleep(0.02) # Throttle idle polling
                else:
